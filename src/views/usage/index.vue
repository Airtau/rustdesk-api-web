  ...
  try {
    ...
    if (res && res.data) {
      ...
      for (const line of lines) {
        ...
        if (match) {
          console.log(`  ✅ [DEBUG] Найдено совпадение! IP=${ip}, time=${secondsAgo}с, created_at=${match.created_at}, diff=${minDiff}ms`)
        } else {
          console.log(`  ⚠️ [DEBUG] Совпадение НЕ найдено для IP=${ip}, time=${secondsAgo}с`)
        }
